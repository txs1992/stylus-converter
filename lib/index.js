...
function isFunctinCallMixin(node) {
  ...
  if (node.__type === 'Call') {
    ...
    var call = nodes.find(function (it) {
      return it.__type === 'Call';
    });
    if (call) {
...
function visitExpression(node) {
  ...
  nodes.forEach(function (node, idx) {
    ...
    if (node.__type === 'Comment') {
      ...
      var symbol = isProperty && node.nodes.length ? ',' : '';
...
function visitor(ast, options, globalVariableList) {
  quote = options.quote;
...
function converter(result) {
  ...
  var text = visitor(ast, options, globalVariableList);
