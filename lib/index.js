...
function visitBlock(node) {
  ...
  //   if (isFunction && item.__type !== 'If' && index + 1 === nodes.length) {
  //     const endExp = visitNode(item)
  //     // const matchs = endExp.match(/\s+/g)
  //     // console.log(`endExp: |${endExp}|`)
  //     // console.log(`endExp: |${endExp.replace(/\s+/, matchs[0] + '@return ')}|`)
  //     // text += `${endExp.replace(/\s+/, matchs[0] + '@return ')}`
  //     text += endExp
  //   } else {
  //     text += visitNode(item)
  //   }
